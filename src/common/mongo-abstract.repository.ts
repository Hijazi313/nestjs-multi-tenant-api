 ...
 * ```
 */
export abstract class MongoEntityRepository<T extends Document> {
  private readonly logger = new Logger(MongoEntityRepository.name);
  ...
  /**
   * Finds a document by its ID. The document is returned as lean object
   * @param id - ID of the document to retrieve.
   * @param options
   * @param projection - Fields to include/exclude from the result.
   * @returns Promise resolving to the found document or null.
   * @throws HttpException with NOT_FOUND status if no document is found.
   */
  async findByIdLean(id: string | Types.ObjectId) {
    const data = await this.options.entityModel.findById(id).lean();
